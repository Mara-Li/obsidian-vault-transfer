...
export function overrideOutputPath(
	path: string,
	settings: VaultTransferSettings,
	metadate?: number | string
) {
...
export function getFolderNote(folder: TFolder, app: App) {
	...
	const outsideFolderNote = app.vault.getAbstractFileByPath(
		normalizePath(`${parentFolder}/${folder.name}.md`)
	);
	if (outsideFolderNote && outsideFolderNote instanceof TFile) return outsideFolderNote;
}

export function getMetadataDate(
	...
	if (
		settings.dateVariable.type === "frontmatter" &&
		settings.dateVariable.frontmatterKey
	) {
		metadataDate =
 ...
 * @param plugin {VaultTransferPlugin}
 * @param outputPath {string} Optional output path to override the settings output path
 */
export async function transferFolder(
	...
	if (!folderNote) {
		...
		const folderInsideNote = app.vault.getAbstractFileByPath(
			`${folder.path}/${folder.name}.md`
		);
		if (folderInsideNote instanceof TFile) {
			folderNote = folderInsideNote;
		} else {
			//maybe index ?
			const folderIndexNote = app.vault.getAbstractFileByPath(`${folder.path}/index.md`);
			if (folderIndexNote instanceof TFile) folderNote = folderIndexNote;
		}
	}
	...
	new Notice(
		`Transfer completed. ${noticeMessage.length} files copied into ${outputPath ?? settings.outputFolder}. See logs for details!`
	);
 ...
 */
export function insertLinkToOtherVault(
	editor: Editor,
	view: MarkdownView,
	settings: VaultTransferSettings
) {
...
function showErrorIfSettingsInvalid(settings: VaultTransferSettings): boolean {
	const message: string | null =
		settings.outputVault.trim().length == 0 ? "Target vault has not been set." : null;
...
function copyAllAttachments(
	...
	for (const attachment of attachments) {
		...
		const attachmentPath = app.metadataCache.getFirstLinkpathDest(
			attachment.link.replace(/#.*/, ""),
			file.path
		);
		if (attachmentPath) {
			//recreate the path of the attachment in the new vault
			let newAttachmentPath = normalizePath(
				`${newVault.replace(file.name, "")}/${attachmentPath.path}`
			);
			//apply override rules to the attachment path
			newAttachmentPath = overrideOutputPath(newAttachmentPath, plugin.settings);
			newAttachmentPath = removePartOfPath(plugin.settings, newAttachmentPath);
			...
			if (!fs.existsSync(newAttachmentPath.replace(attachmentPath.name, ""))) {
				...
				fs.mkdirSync(newAttachmentPath.replace(attachmentPath.name, ""), {
					recursive: true,
				});
			}
			//copy the attachment
